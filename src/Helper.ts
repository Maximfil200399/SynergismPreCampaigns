...
export const addTimers = (input: TimerInput, time = 0) => {
  ...
  switch (input) {
      ...
      if (player.octeractTimer >= 1) {
        ...
        if (player.highestSingularityCount >= 160) {
          ...
          for (let i = 0; i < amountOfGiveaways; i++) {
            const quarkFraction = (player.quarksThisSingularity - player.quarksConverted) * frac * actualLevel
            player.goldenQuarks += quarkFraction * calculateGoldenQuarkGain(true)
            player.quarksConverted += quarkFraction
          }
        }
    ...
    case 'ambrosia': {
      ...
      const maxAccelMultiplier = (30/60)
        + (6/60) * +(player.singularityChallenges.noAmbrosiaUpgrades.completions >= 15)
        + (4/60) * +(player.singularityChallenges.noAmbrosiaUpgrades.completions >= 19)
        + (5/60) * +(player.singularityChallenges.noAmbrosiaUpgrades.completions >= 20)
